...
team_outcome_bayes <- team_outcome %>%
  ...
  mutate(
    ...
    beta_sim = rbeta(n = cumulative_match_num, shape1 = alpha + cumulative_wins, shape2 = beta + cumulative_match_num- cumulative_wins),
    Variance = (team_alpha * team_beta) / ((team_alpha + team_beta)^2 * (team_alpha + team_beta + 1)),
    SD = sqrt(Variance)) 

# rbeta(n = nsim, shape1 = alpha + W, shape2 = beta + L)

team_outcome_bayes %>% View()

xc = team_outcome_bayes %>% dplyr::filter(team == 'Reinder Nummerdor, Richard Schuil')
plot(xc$Un_Adj_Win_Pct)
plot(xc$Bayes_Adj_Win_Pct)
plot(xc$Un_Adj_Win_Pct, xc$Bayes_Adj_Win_Pct)
```
...
male_outcome <- team_outcome_bayes %>% filter(gender == "M",
                                        cumulative_match_num >= 10)
female_outcome <- team_outcome_bayes %>% filter(gender == "W",
                                          cumulative_match_num >= 10)
