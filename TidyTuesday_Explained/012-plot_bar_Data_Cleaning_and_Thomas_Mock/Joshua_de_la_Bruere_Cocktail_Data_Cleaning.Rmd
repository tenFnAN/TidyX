...
cleaned2 = cleaned2 %>%
  dplyr::mutate(
    measure = gsub('(0.50.5)|(0.5.75)|0.5.5', '0.50', measure),
    measure2 = ifelse(str_detect(measure, '\\/'), measure, NA) 
  ) %>%
  dplyr::rowwise() %>%
  dplyr::mutate( 
    measure3 = eval(parse(text = measure2))
  ) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(
    measure_n = as.numeric(measure), 
    measure = case_when(!is.na(measure_n) ~ measure_n,
                         T ~ measure3)
  )
 

cleaned2 %>%
  mutate(measure = as.numeric(tidyr::replace_na(measure, 1))) %>%
  mutate(
    in_oz= case_when(
